...
describe('CitationSearch More menu', () => {
  ...
  afterEach(cleanup);
  it('renders the three base tabs plus an icon-only More tab', () => {
    render(<CitationSearch />);
    const tabs = screen.getAllByRole('tab');
    expect(tabs.map((t) => t.textContent)).toEqual(['Website', 'Book', 'Journal', 'More source types']);
  });
  ...
  it('opens the menu on More click and activates the chosen source type', () => {
    ...
    const moreTab = screen.getAllByRole('tab')[3];

...
<CitationExampleTable
  source={{ id: 'chatgpt-mansfield', type: 'webpage', title: '“Describe the theme of nature in Jane Austen’s <i>Mansfield Park</i>” prompt', 'container-title': 'ChatGPT', publisher: 'OpenAI', issued: { 'date-parts': [[2024, 9, 23]] }, accessed: { 'date-parts': [[2026, 7, 4]] }, URL: 'https://chatgpt.com/share/66f1b0a0-d704-8000-be9a-85f53c850607' }}
  overrides={{
...
One decision splits the styles more than any other: **who takes the author slot**. MLA leaves it empty and leads with the prompt, because "We do not recommend treating the AI tool as an author." APA credits the company, OpenAI, exactly as it would a corporate author. Chicago credits the tool itself, ChatGPT, with OpenAI as publisher. AMA agrees with MLA that the AI is no author but reaches the opposite conclusion about the entry: its manual cites the tool like software — name, version, company, year — and keeps the prompt out of the reference entirely. Harvard, Vancouver, and IEEE have no dedicated AI rule yet, so for those the entry treats the share page as a web page whose title is the prompt — the prompt-as-title pattern above.
...
- **Harvard:** the shortened title and year, since the entry has no author

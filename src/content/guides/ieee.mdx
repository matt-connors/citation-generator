...
The entries below show each common source type formatted exactly as the *IEEE Reference Guide* prescribes — and exactly as this site's generator emits them. In your own document, render the citation number in a fixed-width left column with the entry text aligned beside it.
...
IEEE references use the abbreviated form of journal and conference titles, italicized. IEEE maintains a list of approved abbreviations; when a journal is not on the list, abbreviate using ISO 4 (drop articles such as "the" and "of"; reduce common words to standard short forms).

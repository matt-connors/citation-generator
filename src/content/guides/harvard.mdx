...
description: 'Complete Harvard referencing guide for Cite Them Right 12, with in-text citations, reference-list rules, source examples, and APA/MLA differences.'
...
Harvard is the author–date style most UK universities, business schools, and social-science departments expect. Unlike APA or MLA, "Harvard" is a family of related conventions rather than a single official standard. This guide follows *Cite Them Right* (Pears and Shields, 2022, 12th edition, Bloomsbury Academic) — the most widely cited authority and the variant [this site's generator](/harvard-referencing-generator/) produces.
...
Harvard's [in-text citation](/guides/in-text-citations/) is **author–date**: the author's surname and the publication year, separated by a comma, in parentheses. With a direct quote, add the page number after another comma. The format points the reader to one — and only one — entry in the Reference list at the back.
...
Entries are alphabetised by the first author's surname. Where there is no named author, alphabetise by the first significant word of the title, ignoring leading *A*, *An*, and *The*. Each entry uses a [hanging indent](/guides/hanging-indents/) — the first line flush left, continuation lines indented — so the eye can scan a column of surnames at the left margin. Spacing is conventionally single-spaced within an entry with a blank line between entries, but double-spacing throughout is acceptable too.

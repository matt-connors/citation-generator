...
- A **clip or full episode uploaded to YouTube** keeps the episode format in MLA, with YouTube taking over the second-container slot — the MLA Style Center has a post on exactly this. If the upload is an unofficial rip, prefer an authorized source; details in the [YouTube guide](/guides/how-to-cite-a-youtube-video/).
...
**The episode is on YouTube.** MLA has an official post on this too: the format holds, with *YouTube* and the video URL replacing the streaming service in the second container. Check that the upload is from the network's official channel before citing it; details in the [YouTube guide](/guides/how-to-cite-a-youtube-video/).
...
**You need Chicago style.** The Chicago Manual of Style's own Q&A (updated for the 18th edition, published September 2024 — note that some university pages still serve 17th-edition guidance) says episodes are usually cited in the notes or running text, with a bibliography entry often unnecessary. Its full-note example:
...
**You need AMA, IEEE, or Harvard.** AMA 11 uses a bracketed media type and appends any URL with no closing period: Benioff CD, Weiss DB. Game of Thrones [Television series]. New York, NY: HBO; 2011. IEEE similarly brackets a descriptor — [Television broadcast] or [Streaming video] — with producer, title, date, network, and URL. Harvard has no single governing body, so follow the pattern in the table above and stay consistent; if your department publishes its own Harvard variant, it wins.

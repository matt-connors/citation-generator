...
pubDate: 2026-03-11
description: 'How in-text citations work across the seven major academic styles. Author–date versus numeric systems, the exact format for each style, page-number rules, and the et al. thresholds that students get wrong most often. Written and reviewed by the MLA Generator Editorial Team.'
category: 'concept'
...
faq:
  ...
  - question: 'Can I include multiple sources in one in-text citation?'
    answer: 'Yes, and it is often the right choice when several sources support the same claim. APA, Chicago author–date, and Harvard separate sources with semicolons: "(Chen, 2021; Lin & Patel, 2022)" in APA, "(Chen 2021; Lin and Patel 2022)" in Chicago. MLA separates with a semicolon as well: "(Chen 47; Lin and Patel 122)." The numeric styles concatenate the numbers: "[1, 2, 5]" in IEEE, "(1,2,5)" or "¹⁻³" in Vancouver and AMA when the range is consecutive. Order is by alphabet for author–date styles and by citation sequence for numeric.'
...
| [IEEE](/guides/ieee) | 3 authors in prose | *Italic* | Up to 6 authors, then et al. |
| [AMA 11](/guides/ama) | 3 authors in prose | *Italic* (body); roman (reference list) | First 3 authors then et al. once total ≥ 7 |

The combinations matter. A three-author source — Goldstein, Ramanathan, and O'Connor — collapses to `(Goldstein et al., 2024)` in APA, MLA, and Chicago author–date, but stays as `(Goldstein, Ramanathan and O'Connor, 2024)` in Harvard. Picking the wrong threshold for your assigned style produces a paper that looks subtly off to a marker who knows the rules.

## No author and no date

When the source has no individual author, every style moves the title to the author slot. APA uses sentence case with a short title for shorter works: `("Cognitive Load and Curriculum Design," 2023)`; italics for whole works: `(*Reading Proficiency and Learning Loss*, 2023)`. MLA uses a short title in quotation marks or italics depending on the work type. Chicago author–date and Harvard follow APA's pattern with their own punctuation.
...
When the date is missing, use `n.d.` (no date) in APA, Chicago, and Harvard. MLA simply omits the date and relies on the access date you record at the end of the reference list entry. The numeric styles do not encounter this case in-text because the citation is just a number.

## Multiple sources in one citation

When several sources support the same claim, put them in one citation rather than ending three consecutive sentences with parentheticals. The styles differ on separator and ordering:

- **APA, Chicago author–date, Harvard:** Semicolon between sources; alphabetical order by first author. `(Chen, 2021; Goldstein et al., 2024; Lin & Patel, 2022)` in APA.
- **MLA:** Semicolon between sources; alphabetical order. `(Chen 47; Lin and Patel 122)`.
- **Vancouver, AMA:** Concatenated numbers in citation order. `(1, 3, 5)`. Consecutive numbers collapse to a range with an en-dash: `(1–3)` or superscript `¹⁻³`.
- **IEEE:** Bracketed list in citation order. `[1], [3], [5]` or `[1]–[3]` for a range.

...
description: 'Find reliable sources, evaluate evidence, integrate quotes and paraphrases, choose the right citation style, and build a polished works cited page.'
category: 'meta'
keywords: ['research and citation', 'how to cite sources', 'evaluating sources', 'choosing citation style', 'avoiding plagiarism', 'integrating sources']
...
## A five-minute source audit

Before a source makes it into your notes, run a short audit and write the answers beside the citation. It is easier to reject a weak source before it becomes part of your outline than after three paragraphs depend on it.

1. **Who is responsible for the claim?** Name the person, organization, journal, agency, or publisher. If the page hides authorship, say so in your notes.
2. **What kind of evidence is it?** Peer-reviewed article, book chapter, government report, dataset, news report, primary document, opinion essay, or background explainer.
3. **How close is it to the original evidence?** A meta-analysis is closer than a blog summary. A government table is closer than a screenshot of that table on social media.
4. **What claim will it support in your paper?** Write one sentence. If you cannot name the claim, you probably do not need the source yet.
5. **Which citation fields are visible now?** Author, title, container, publisher, date, DOI or URL, and access date when your style requires one.

That last line is not clerical busywork. It catches the page with no date, the report PDF with no listed author, and the article title that differs between the headline and the browser tab before those details become harder to recover.
...
The [works cited page](/guides/works-cited-page/) (MLA), reference list (APA, Harvard, Chicago author–date), or bibliography (Chicago notes–bibliography) is the part of your paper a grader checks first when spot-checking for citation rigor. Two minutes of consistency work here changes how the whole paper reads.
